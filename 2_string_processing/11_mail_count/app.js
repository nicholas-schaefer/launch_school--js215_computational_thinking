...
function mailCount(emailData) {
  const emails = processEmailData(emailData);
  const emailCount = emails.length;

  const dates = emails.map(email => new Date(email.date));
  const sortedDates = [...dates].sort((a,b)=> a - b);

  const emailOldestDate = sortedDates[0];
  const emailNewestDate = sortedDates.slice(-1)[0];

  const formattedEmailOldestDate = formatDate(emailOldestDate);
  const formattedEmailNewestDate = formatDate(emailNewestDate);
  const formattedEmailDateRange = [formattedEmailOldestDate, formattedEmailNewestDate].join(' - ');

  const msg = `Count of Email: ${emailCount}` + '\n' +
              `Date Range: ${formattedEmailDateRange}`;

  return msg;
}

function processEmailData(emailData) {
  const emailArrays = emailData.split('##||##');

  const emails = emailArrays.map( email => {
    const emailPartsArray = email.split('#/#').map(part => part.trim())
    const emailPartsObj = {};
    emailPartsObj.from =      emailPartsArray[0].replace('From:', '').trim();
    emailPartsObj.subject =   emailPartsArray[1].replace('Subject:', '').trim();
    emailPartsObj.date =      emailPartsArray[2].replace('Date:', '').trim();
    emailPartsObj.recipient = emailPartsArray[3].replace('To:', '').trim();
    emailPartsObj.body =      emailPartsArray[4].trim();

    return emailPartsObj
  })
  return emails
}
